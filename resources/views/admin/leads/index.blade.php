    ...
    <div class="row">
        <div class="col-12">
            <ul class="nav nav-tabs nav-bordered">
              <li class="nav-item">
                  <a href="{{ url($route.'/leads')}}"  class="nav-link px-3 py-2 {{ request()->get('status')=='' ? 'active' : '' }}">
              ...
              @php
              $name = str_replace('_', '-', $statu->name);
              @endphp
              <li class="nav-item" style="color:{{ $statu->color_code }};background-color: {{ $statu->color_code_opcity}}">
                  <a href="{{ url($route.'/leads?status='.$statu->name)}}"  class="nav-link px-3 py-2 {{ request()->get('status')== $statu->name ? 'active' : '' }}">
                      <i class="mdi mdi-pencil-box-multiple font-18 d-md-none d-block"></i>
                      <span class="d-none d-md-block">{{ucfirst($name)}}</span>
                  </a>
              </li>
              @endforeach
          </ul>
                                    ...
                                    <td>
                                      <select style="display: none;" class="form-control application-status" data-lead_id="{{$user->id}}" data-status_id="{{ ($user->application)?$user->application->status_id:'' }}" data-application_id="{{ ($user->application)?$user->application->id:'' }}"   id="application_status_{{$user->id}}">
                                        @foreach($status as $data)
                                          <option style="color:{{ $data->color_code }};background-color: {{ $data->color_code_opcity}}" value="{{$data->id}}" @if($user->application->status == $data->name)  selected @endif>{{$data->name}}</option>
                                        @endforeach
                                      </select>
